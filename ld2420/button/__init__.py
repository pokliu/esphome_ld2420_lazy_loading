...
LD2420InitModuleButton = ld2420_ns.class_("LD2420InitModuleButton", button.Button)

CONF_APPLY_CONFIG = "apply_config"
CONF_REVERT_CONFIG = "revert_config"
CONF_RESTART_MODULE = "restart_module"
CONF_FACTORY_RESET = "factory_reset"
CONF_INIT_MODULE = "init_module"
...
CONFIG_SCHEMA = {
    ...
    cv.Optional(CONF_INIT_MODULE): button.button_schema(
        LD2420InitModuleButton,
        device_class=DEVICE_CLASS_RESTART,
        entity_category=ENTITY_CATEGORY_CONFIG,
        icon=ICON_RESTART,
    ),
...
async def to_code(config):
    ...
    if init_config := config.get(CONF_INIT_MODULE):
        b = await button.new_button(init_config)
        await cg.register_parented(b, config[CONF_LD2420_ID])
        cg.add(ld2420_component.set_init_module_button(b))
